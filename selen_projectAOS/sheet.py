...
class AOS_Sheet:
    ...
    # save random password in a specific test number in XL table
    def generate_password(self,test_number):
        ...
        # combine the data
        low_letter=random.choice(lower)
        up_letter = random.choice(upper)
        num_letter = random.choice(num)
        symbol_letter = random.choice(symbols)
        all = lower + upper + num + symbols
        # use random
        temp = random.sample(all, 4)
        # create the password
        password = "".join(temp)+low_letter+up_letter+num_letter+symbol_letter
        # save the new password in the table
        self.sheet[f"{self.tests[test_number - 1] + '18'}"]=password
        self.workbook.save("Tests_AOS.xlsx")

    # save random username in a specific test number in XL table
    ...
    # save random mail in a specific test number in XL table
